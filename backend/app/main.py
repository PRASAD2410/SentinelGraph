...
def assistant(payload:Question):
 provider=get_llm_provider(); question=payload.question.lower(); persons=[n['label'] for n in nodes.values() if n['type']=='Person']; context={'entities':analytics()[:20],'leads':leads()[:12],'reports':[{'id':r['id'],'title':r['title'],'date':r['date'],'source':r['source'],'full_text':r['text'][:250000]} for r in reports[-3:]]}
